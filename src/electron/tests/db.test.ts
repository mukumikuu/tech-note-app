...
describe('Database Initialization', () => {
  ...
  describe('initDB', () => {
    ...
    it('creates all required tables and triggers', () => {
      ...
      expect(sql).toContain('DROP TRIGGER IF EXISTS blocks_ai')
      expect(sql).toContain('DROP TRIGGER IF EXISTS blocks_ad')
      expect(sql).toContain('DROP TRIGGER IF EXISTS blocks_au')
      expect(sql).toContain('CREATE TRIGGER blocks_ai')
      expect(sql).toContain('CREATE TRIGGER blocks_ad')
      expect(sql).toContain('CREATE TRIGGER blocks_au')
    })
  })
})
